...
my $LAST_TIME_LOW = 3;
my $LAST_TIME_LAST = 4;
my $LAST_TIME_CHANGE = 5;
my $LAST_TIME_PERCHANGE = 6;
my $LAST_TIME_VOLUME = 7;
my $LAST_TIME_TIME = 8;
my $LAST_TIME_DATE = 9;
...
my $driver   = "SQLite";
my $database_vol = "incheck_volume.db";
my $dsn = "DBI:$driver:dbname=$database_vol";
my $userid = "";
my $password = "";

my $repeat_always = 1;
my $delay_in_seconds = $simulation == 1? 5: 60;
...
while ($repeat_always) {
	...
	if (check_for_download_time()) {
		if ($simulation) {
			system("cp inrealtime_copy.db $database_vol");
		} else {
			system("cp inrealtime_data.db $database_vol");
		}
		...
		foreach my $stock_name (sort keys %cur_price)
		{
			...
			if (defined $fno_market_lot{$stock_name} and $fno_market_lot{$stock_name} ne '') {
				$quantity = $fno_market_lot{$stock_name};
			} else {
				$quantity = floor($lot_size_in_cash/$stock_price);
			}
				...
				if ($trade_trigerred) {
					...
					if (!defined $open_trade_type{$stock_name} or $open_trade_type{$stock_name} ne $recommendation) {
						...
						if ($new_trade) {
							...
							my $email_cmd = qq(-s "$company_name $report_simulation $time_of_last_price{$stock_name}: $email_recommendation $stock_name \($stock_price\) Target: $profit_price StopLoss: $stop_loss_price" $email_list < /dev/null);
							...
							my $email_cmd = qq(-s "$company_name $report_simulation $time_of_last_price{$stock_name}: $email_recommendation $stock_name \($stock_price\) Porfit: $profit_loss_u Max_Drawdown: $max_loss Max_Profit: $max_profit" $email_list < /dev/null);
							my $email_sent = $send_email == 1? system("$email_program $email_cmd") : 0;
							...
							$email_cmd = qq(-s "$company_name $report_simulation $time_of_last_price{$stock_name}: $email_recommendation $stock_name \($stock_price\) Target: $profit_price StopLoss: $stop_loss_price" $email_list < /dev/null);
			...
			if (defined $last_row[$STOCK_ID] and defined $open_trade_type{$stock_name}) {
				...
				my $profit_loss = 0.0;
				my $stop_profit = 0.0;
				#my $quantity = floor($lot_size_in_cash/$cur_price{$stock_name});
				if (defined $fno_market_lot{$stock_name} and $fno_market_lot{$stock_name} ne '') {
					$quantity = $fno_market_lot{$stock_name};
				} else {
					$quantity = floor($lot_size_in_cash/$open_trade_entry_price{$stock_name});
				...
				if ($trade_status eq "CLOSED") {
					...
					my $email_recommendation = "CLOSED";
					my $email_cmd = qq(-s "$company_name $report_simulation $time_of_last_price{$stock_name}: $email_recommendation $stock_name \($stock_price\) Profit: $profit_loss_u Max_Drawdown: $max_loss Max_Profit: $max_profit" $email_list < /dev/null);
					my $email_sent = $send_email == 1? system("$email_program $email_cmd") : 0;
				}
